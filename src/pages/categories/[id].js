...
const sites = [
  {
    ...
    description:
      "Nishantha is a global marketplace for all your products and services. We offer a wide range of products and services that are available to you.",
  },
  {
    id: 2,
    name: "Nishantha",
...
const Categories = () => {
  ...
  useEffect(() => {
    const findNish = data?.filter((item) => item._id === id);
    setnish(findNish[0]);
  }, [id]);
  //   console.log(nish);
  return (
    <>
      ...
      <main className="">
        ...
        <div className="my-10 w-[80%] mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
          {sites.map((site) => (
            <SingleCard key={site.id} site={site} />
          ))}
        </div>
      </main>
    </>
